...
from andy_axcel_ia.config import CACHE_DIR
from andy_axcel_ia.constants import ERROR_MESSAGES
from andy_axcel_ia.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from andy_axcel_ia.routers.files import upload_file
from andy_axcel_ia.utils.auth import get_admin_user, get_verified_user
from andy_axcel_ia.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-OpenAndy Axcel IA-User-Name"] = quote(user.name, safe=" ")
                headers["X-OpenAndy Axcel IA-User-Id"] = user.id
                headers["X-OpenAndy Axcel IA-User-Email"] = user.email
                headers["X-OpenAndy Axcel IA-User-Role"] = user.role
