...
from andy_axcel_ia.utils.payload import (
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                ...
                headers={
                    ...
                    **(
                        {
                            "X-OpenAndy Axcel IA-User-Name": quote(user.name, safe=" "),
                            "X-OpenAndy Axcel IA-User-Id": user.id,
                            "X-OpenAndy Axcel IA-User-Email": user.email,
                            "X-OpenAndy Axcel IA-User-Role": user.role,
                        }
...
async def send_post_request(
    ...
    try:
                ...
                **(
                    {
                        ...
                        "X-OpenAndy Axcel IA-User-Role": user.role,
                        **(
                            {"X-OpenAndy Axcel IA-Chat-Id": metadata.get("chat_id")}
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"Andy Axcel IA Abierta de Andy Dominguez: Server Connection Error",
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else "Andy Axcel IA Abierta de Andy Dominguez: Server Connection Error",
        )
    finally:
        if not stream:
            await cleanup_response(r, session)
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Andy Axcel IA Abierta de Andy Dominguez: Server Connection Error"
            )
        except Exception as e:
    ...
    if url_idx is None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else "Andy Axcel IA Abierta de Andy Dominguez: Server Connection Error",
            )
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "Andy Axcel IA Abierta de Andy Dominguez: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
