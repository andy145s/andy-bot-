<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_andy_axcel_ia_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'Andy Axcel IA Abierta de Andy Dominguez version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						OPEN_WEBUI_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_andy_axcel_ia_version ?? '0.0.0'
					}
				)
			);
			return;
